function [G_updown,G_lateral,G_tangential,path_length] = coaster_brakingsection(h_0,inc)
...
a_b = 30; % chosen backwards deceleration
db = (v0^2) / (2 * (a_b)); % distance traveled during braking
g = 9.81; % gravitational acceleration in m/s^2
...
G_tangential = -(a_b / g) * ones(size(path_length)); % constant backward G's
...
subplot(3,1,3)
hold on
plot(path_length,G_tangential,'b',LineWidth=1.5)
yline(5,'r')
yline(-4,'r')
hold off
...
print('brakingsection_gforces_2803_lab1','-dpng','-r300') %saves image file (png)
